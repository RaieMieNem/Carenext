...
export const useFormLogicProjet = () => {
  // États de visibilité 


  useEffect(() => {
    // Récupération des éléments du DOM
    const StructureSelect = document.getElementById("structure-select") as HTMLSelectElement;
    const scipart = document.getElementById("scipart") as HTMLElement;
    ...
    // Affiche la case "Nombre de part(s)" si "SCI" est sélectionné
    if (StructureSelect) {
      StructureSelect.addEventListener("change", () => {
        toggleVisible(scipart, StructureSelect.value === "sci");
      });
    }
    ...
    if (financementSelect) {
      financementSelect.addEventListener("change", () => {
        ...
        const toggleAmortissement = () => {
            ...
            const isCredit = financementSelect.value === "credit";
            const shouldShow = num > 0 && isCredit;
